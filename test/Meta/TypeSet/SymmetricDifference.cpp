...
#include <Meta/TypeSet/SymmetricDifference.hpp>
...
namespace {
	...
	void twoTest() {
		...
		{ // Some same types
			constexpr auto value = Test::checkBinaryTypeOperator<
				...
				SetType<Type<2>, Type<4>, Type<6>, Type<8>>
			>();
	...
	TEST(Meta_TypeSet_SymmetricDifference, Two) {
		#define X(type) twoTest<Meta::TypeSet::SymmetricDifference, type>();
		#include <Test/SetTypes.xmacro>
	}

	TEST(Meta_TypeSet_SymmetricDifference, Multiple) {
		#define X(type) multipleTest<Meta::TypeSet::SymmetricDifference, type>();
		#include <Test/SetTypes.xmacro>
	}
}
